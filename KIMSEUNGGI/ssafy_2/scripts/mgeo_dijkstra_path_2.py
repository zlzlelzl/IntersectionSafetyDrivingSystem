...
class dijkstra_path_pub :
    ...
    def init_callback(self,msg):
        ...
        # tmp.pose.pose.position.x
        input_x = msg.pose.pose.position.x
        input_y = msg.pose.pose.position.y
        input_z = msg.pose.pose.position.z
        node_min_distance = float('inf')

        for k,v in self.nodes.items():
            node_xyz = v.to_dict()['point']
            tmp_distance = sqrt((node_xyz[0] - input_x)**2 + (node_xyz[1] - input_y)**2 + (node_xyz[2] - input_z)**2)
            if node_min_distance > tmp_distance :
                node_min_distance = tmp_distance
                node_idx = k
    ...
    def goal_callback(self,msg):
        ...
        '''
        input_x = msg.pose.position.x
        input_y = msg.pose.position.y
        input_z = msg.pose.position.z
        node_min_distance = float('inf')
