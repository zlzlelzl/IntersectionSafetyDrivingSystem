...
def Object_callback(data):
    ...
    for i in range(data.num_of_pedestrian) :
        rospy.loginfo('--------------------Num {}-------------------------'.format(i))
        npc = data.pedestrian_list[i]
        rospy.loginfo('name : {}'.format(npc.name))
        rospy.loginfo('position     : x = {0} , y = {1}, z = {2}'.format(npc.position.x,npc.position.y,npc.position.z))
        rospy.loginfo('velocity     : x = {0} , y = {1}, z = {2} m/s^2'.format(npc.velocity.x,npc.velocity.y,npc.velocity.z))
        rospy.loginfo('acceleration : x = {0} , y = {1}, z = {2} m/s'.format(npc.acceleration.x,npc.acceleration.y,npc.acceleration.z))
        rospy.loginfo('heading      : {} deg'.format(npc.heading))
        rospy.loginfo('size         : x = {0} , y = {1}, z = {2} m'.format(npc.size.x,npc.size.y,npc.size.z))
        rospy.loginfo('name : {}'.format(npc.position.x, npc.position.y, npc.position.x))

    rospy.loginfo('-------------------- Obstacle -------------------------')
    rospy.loginfo('NPC num :{}'.format(data.num_of_obstacle))
    for i in range(data.num_of_obstacle) :
        rospy.loginfo('--------------------Num {}-------------------------'.format(i))
        npc = data.obstacle_list[i]
        rospy.loginfo('name : {}'.format(npc.name))
        rospy.loginfo('position     : x = {0} , y = {1}, z = {2}'.format(npc.position.x,npc.position.y,npc.position.z))
        rospy.loginfo('velocity     : x = {0} , y = {1}, z = {2} m/s^2'.format(npc.velocity.x,npc.velocity.y,npc.velocity.z))
        rospy.loginfo('acceleration : x = {0} , y = {1}, z = {2} m/s'.format(npc.acceleration.x,npc.acceleration.y,npc.acceleration.z))
