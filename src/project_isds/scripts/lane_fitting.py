...
class IMGParser:
    def __init__(self, pkg_name='ssafy_3'):
        ...
        self.crop_pts = np.array(
            [[[0, 480], [0, 350], [280, 200], [360, 200], [640, 350], [640, 480]]])
        ...
        curve_learner = CURVEFit(
            order=3, lane_width=3.5, y_margin=1, x_range=30, min_pts=50)
        ...
        while not rospy.is_shutdown():
            ...
            if self.img_bgr is not None and self.is_status == True:
                bin_img = self.binarize(self.img_bgr)

                img_crop = self.mask_roi(bin_img)
                img_warp = bev_op.warp_bev_img(img_crop)

                img_f = bev_op.warp_inv_img(img_warp)
                ...
                lane_path = curve_learner.write_path_msg(
                    x_pred, y_pred_l, y_pred_r)
...
class BEVTransform:
    ...
    def warp_inv_img(self, img_warp):
        img_f = cv2.warpPerspective(
            img_warp, self.perspective_inv_tf, (self.width, self.height), flags=cv2.INTER_LINEAR)
...
class CURVEFit:
    def __init__(self, order=3, alpha=10, lane_width=4, y_margin=0.5, x_range=5, dx=0.5, min_pts=50):
        ...
        self.lane_path = Path()
        # curve_learner = CURVEFit(order=0, lane_width=3,y_margin=5.0, x_range=3, min_pts=50)
    ...
    def _init_model(self):
        X = np.stack(
            [np.arange(0, 2, 0.02)**i for i in reversed(range(1, self.order+1))]).T
    ...
    def preprocess_pts(self, lane_pts):
        ...
        for d in np.arange(0, self.x_range, self.dx):
            ...
            idx_full_list = np.where(np.logical_and(
                lane_pts[0, :] >= d, lane_pts[0, :] < d+self.dx))[0].tolist()

            idx_list += random.sample(idx_full_list,
                                      np.minimum(self.min_pts, len(idx_full_list)))
        ...
        y_right = y_g[np.logical_and(
            y_g >= y_ransac_collect_r-self.y_margin, y_g < y_ransac_collect_r+self.y_margin)]
        ...
        y_left = y_g[np.logical_and(
            y_g >= y_ransac_collect_l-self.y_margin, y_g < y_ransac_collect_l+self.y_margin)]
    ...
    def write_path_msg(self, x_pred, y_pred_l, y_pred_r, frame_id='/map'):
        ...
        trans_matrix = np.array([
                                [math.cos(
                                    self.vehicle_yaw), -math.sin(self.vehicle_yaw), self.vehicle_pos_x],
                                [math.sin(self.vehicle_yaw),  math.cos(
                                    self.vehicle_yaw), self.vehicle_pos_y],
                                [0, 0, 1]])
        ...
        for i in range(len(x_pred)):
            ...
            local_result = np.array(
                [[x_pred[i]], [(0.5)*(y_pred_l[i] + y_pred_r[i])], [1]])
    ...
    def set_vehicle_status(self, vehicle_status):
        ...
        _, _, vehicle_yaw = tf.transformations.euler_from_quaternion(
            odom_quaternion)
